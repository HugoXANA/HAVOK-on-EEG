...
% 读取数据
xdat = readNPY('S01_prop_45hz.npy');
xdat = xdat';
% size(xdat): T, dim

% 截取感兴趣数据
% xdat = xdat(900000:1260000, :);
% xdat = xdat(1020000:1260000, :);
xdat = xdat(900000:1140000, :);
N = length(xdat);
sample_frep = 2048;
dt = 1/sample_frep;
tspan = [dt:dt:N];
...
L = 1:size(x,1);
u_seq = x(L,r);
% u_seq = zeros(size(x,1),1);
[y,t] = lsim(sys,u_seq,dt*(L-1),x(1,1:r-1));
...
set(gca,'XTick',[1:1.e4:length(xdat)])
axis on
...
tsubplot(n,1,n,type);
plot(L,u_seq,'Color',[.5 0 0],'LineWidth',1.5)
xlabel('t'), ylabel('v_r')
xlim([start_ind end_ind])
box on, axis on
axis tight
